...
const mediaApi = {
    ...
    async getLyrics(query) {
        // 1. Try LRCLIB (Primary - High Quality)
        try {
            const { data } = await axios.get(`https://lrclib.net/api/search?q=${encodeURIComponent(query)}`, AXIOS_OPTS);
            if (data && data.length > 0) {
                // 🕵️ SMART SEARCH: Find the best match by checking artist/title context
                const queryLower = query.toLowerCase();
                let bestMatch = data.find(m => 
                    m.plainLyrics && 
                    (queryLower.includes(m.artistName.toLowerCase()) || queryLower.includes(m.trackName.toLowerCase()))
                ) || data[0];
        ...
        // 2. Fallback: Nabees API
        try {
            console.log(`📡 Trying Nabees Lyrics Fallback for: ${query}`);
            const { data } = await axios.get(`https://api.nabees.online/api/lyrics?q=${encodeURIComponent(query)}`, AXIOS_OPTS);
            if (data && data.result) {
                return {
            ...
            console.error('Nabees Lyrics error:', error.message);
        }
