...
const loadCommands = () => {
    ...
    for (const file of files) {
        ...
        try {
            ...
            // 2. Scan for other command objects in the same file
            Object.values(module).forEach(val => {
                if (typeof val === 'object' && val !== null && val !== module) {
                    register(val);
                }
            });
...
async function handleMessages(sock, { messages, type }) {
    ...
    if (!command) {
        ...
        // ❌ Unknown Command Feedback (Only if prefix was used)
        if (commandName && prefix) {
            return await sock.sendMessage(jid, { 
                text: `❌ *Unknown Command:* Sorry, I don't recognize *${prefix}${commandName}*.\n\nType *.menu* to see all available commands!` 
            }, { quoted: msg });
        }
        return;
    }
    ...
    console.log(`🏁 Command handling finished for ${commandName}`);
        ...
        console.error("🏁 Critical Error in handleMessages:", err);
    }
}
